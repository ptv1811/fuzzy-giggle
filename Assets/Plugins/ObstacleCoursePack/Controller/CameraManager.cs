...
public class CameraManager : MonoBehaviour {
	...
	void FollowTarget (float d) { //Function that makes the camera follow the player
	...
	void HandleRotations (float d, float v, float h, float targetSpeed) { //Function that rotates the camera correctly
		if (turnSmoothing > 0) {
	...
	private void FixedUpdate () { //Function that correctly rotates the camera based on the joystick / mouse and follows the player (the delta time is sent to be independent of the fps)
